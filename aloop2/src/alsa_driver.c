...
#include <sys/time.h> // added by Artem
//#include "engine.h"
...
#if 1// added Artem
#define MESSAGE jack_info
#define jack_error jack_info
void
		  ...
		  *delayed_usecs)
{
  ...
  again:
	...
	*/
	...
	return avail - (avail % driver->frames_per_cycle); // need to remove division
}
...
jack_get_microseconds_from_system (void) {
	...
	jackTime = (jack_time_t) time.tv_sec * 1000000 + (jack_time_t) time.tv_nsec / 1000;
	return jackTime;
}
...
alsa_driver_initialize()
{
	...
	if(driver)
	{
		driver->get_microseconds = jack_get_microseconds_from_system;
	}
...
void
alsa_driver_loop(alsa_driver_t* driver, process_t* process)
{
	jack_nframes_t nframes = 1;
	int wait_status;
	float delayed_usecs;

	if(	0 != alsa_driver_start(driver) )
	{
		return;
	}
	...
	while(1)
	{
		...
		{
			printf("-");
			alsa_driver_read(driver, nframes);
		}

		if(driver->playback_handle)
		{
			printf("+");
			alsa_driver_write(driver, nframes);
		}
