...
#define DEFAULT_BUFFER_SIZE     (8*DEFAULT_PERIOD_SIZE)
...
int start_audio(Audio_t* audio)
{
    ...
    if(!open_stream(&audio->handle_capture, audio->device_name, SND_PCM_STREAM_CAPTURE, &audio->sample_rate, audio->period_size, audio->buffer_size)) {
    	return 0;
    }
    printf("Opened capture stream with %d sample rate\n", audio->sample_rate);
    if(!open_stream(&audio->handle_playback, audio->device_name, SND_PCM_STREAM_PLAYBACK, &audio->sample_rate, audio->period_size, audio->buffer_size)) {
    	return 0;
    }
    printf("Opened playback stream with %d sample rate\n", audio->sample_rate);
...
static int get_options(Audio_t* audio, int argc, char *argv[])
{
    ...
    if (needhelp) {
        printf(
                "Usage: aloop [OPTIONS]\n"
                "-h,--help      this message\n"
                "-d,--device    playback/capture device (plughw:0,0 by default - good for internal devices, try plughw:1,0 for others)\n"
...
int main(int argc, char* argv[])
{
    ...
    printf("Starting audio device %s with %d sample rate, %d period size, %d buffer size\n",
